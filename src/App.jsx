        ...
        <section className="vaccine-stock-section">
          ...
          <div className="vaccine-view vaccine-overview-view">
            <div className="stats-grid vaccine-kpis">
              ...
              <KpiCard icon={ChartNoAxesCombined} label="Average monthly issues" value={Math.round(vaccineSummary.consumption).toLocaleString()} sub="Reported provincial AMI in the current filter" tone="amber" onClick={() => setVaccineWorkspaceView("consumption")} title="Open AMI trend" />
          ...
          <div className="vaccine-view vaccine-stock-view">
            ...
            <div className="table-panel"><div className="table-headline"><div><h2>Stock on hand by antigen</h2><p>Use this view to compare reported stock, AMI, and months of stock.</p></div></div><div className="table-scroll"><table><thead><tr><th>Vaccine</th><th>Stock on hand</th><th>AMI</th><th>Average MOS</th><th>District records</th></tr></thead><tbody>{[...vaccineProductRows].sort((a, b) => b.stock - a.stock).map((row) => <tr key={row.name}><td><strong>{row.name}</strong></td><td>{Math.round(row.stock).toLocaleString()}</td><td>{Math.round(row.amc).toLocaleString()}</td><td>{formatMos(row.mos)}</td><td>{row.rows.length.toLocaleString()}</td></tr>)}</tbody></table></div></div>
          </div>
