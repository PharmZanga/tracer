...
const dashboardPages = [
  ...
  { id: "vaccines", short: "VX", label: "Vaccine Stock", icon: Syringe },
...
const moduleDescriptions = {
  ...
  vaccines: "Monthly provincial vaccine stock, AMI, months of stock, and expiry monitoring.",
...
function vaccineExpiryDate(value) {
  const text = String(value ?? "").trim();
  if (!text || text === "-" || text === "0") return null;
  const iso = text.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);
  if (iso) return new Date(Date.UTC(Number(iso[1]), Number(iso[2]), Number(iso[3])));
  const monthYear = text.match(/^(\d{1,2})\D+(\d{2,4})$/);
...
function vaccinePeriodEnd(period) {
  const match = String(period?.id || "").match(/^(\d{4})-(\d{2})$/);
  return match ? new Date(Date.UTC(Number(match[1]), Number(match[2]), 0)) : null;
}
...
function KpiCard({ label, value, sub, tone = "green", icon: Icon, onClick, active = false, title }) {
  ...
  return (
    <Tag className={`stat stat-${tone}${active ? " active" : ""}`} type={onClick ? "button" : undefined} onClick={onClick} aria-pressed={onClick ? active : undefined} title={title}>
      <span>{Icon ? <Icon className="stat-icon" aria-hidden="true" /> : null}{label}</span>
      <strong>{value}</strong>
      <small>{sub}</small>
    </Tag>
  );
...
function VaccineBarChart({ title, subtitle, rows, value, label, format = (entry) => Math.round(entry).toLocaleString(), tone = "green", onSelect }) {
  const chartRows = rows.filter((row) => Number.isFinite(value(row)) && value(row) >= 0);
  const maximum = Math.max(1, ...chartRows.map((row) => value(row)));
  return <div className={`table-panel vaccine-chart tone-${tone}`}>
    <div className="table-headline"><div><h2>{title}</h2><p>{subtitle}</p></div><span>{chartRows.length} points</span></div>
    <div className="vaccine-bar-chart">
      {chartRows.map((row) => {
        const current = value(row);
        const Tag = onSelect ? "button" : "div";
        return <Tag className="vaccine-bar" type={onSelect ? "button" : undefined} onClick={onSelect ? () => onSelect(row) : undefined} key={row.id || row.name || row.label} title={`${label(row)}: ${format(current)}`}>
          <span className="vaccine-bar-value">{format(current)}</span>
          <i style={{ height: `${Math.max(5, Math.round((current / maximum) * 100))}%` }} />
...
function App() {
  ...
  const [vaccinePeriodId, setVaccinePeriodId] = useState(() => vaccineStockData.periods.at(-1)?.id || "");
  const [vaccineProvince, setVaccineProvince] = useState("all");
  ...
  const vaccineWorkspace = useMemo(() => {
    ...
    if (activePage !== "vaccines") return { selectedVaccinePeriod, vaccineProvinceOptions, vaccineDistrictOptions, vaccineNameOptions, vaccineRows: [], vaccineSummary: { districts: 0, availability: 0, mos: null, stockouts: 0, stock: 0, districtAmc: 0, provincialAmi: 0, expiring: 0, expired: 0, nonReporting: 0 }, vaccineTrendRows: [], vaccineProductRows: [], vaccineExpiryRows: [], vaccineStockoutRows: [], vaccineNonReportingDistricts: [], vaccineReportingUnits };
    ...
    const vaccineDistrictRows = vaccineRows.filter((row) => row.reportingUnitType === "district");
    const vaccineProvincialRows = vaccineRows.filter((row) => row.reportingUnitType === "provincial_office");
    ...
    const expiryWindowEnd = selectedPeriodEnd ? new Date(Date.UTC(selectedPeriodEnd.getUTCFullYear(), selectedPeriodEnd.getUTCMonth() + 7, 0)) : null;
    const vaccineExpiryRows = vaccineRows
      .map((row) => ({ ...row, parsedExpiry: vaccineExpiryDate(row.expiryDate) }))
      .filter((row) => row.stock > 0 && row.parsedExpiry)
      .map((row) => ({ ...row, expiryStatus: row.parsedExpiry < selectedPeriodEnd ? "Expired" : row.parsedExpiry <= expiryWindowEnd ? "Expiring within 6 months" : "Valid" }))
    ...
    const vaccineSummary = {
      ...
      stock: vaccineRows.reduce((sum, row) => sum + (Number(row.stock) || 0), 0),
      districtAmc: vaccineDistrictRows.reduce((sum, row) => sum + (Number(row.amc) || 0), 0),
      provincialAmi: vaccineProvincialRows.reduce((sum, row) => sum + (Number(row.amc) || 0), 0),
      expiring: vaccineExpiryRows.filter((row) => row.expiryStatus === "Expiring within 6 months").length,
      expired: vaccineExpiryRows.filter((row) => row.expiryStatus === "Expired").length,
    ...
    const vaccineTrendRows = vaccineStockData.periods.map((period) => {
      ...
      return { id: period.id, label: period.label, sourceSheet: period.sourceSheet, availability: rows.length ? rows.filter((row) => row.stock > 0).length / rows.length : null, mos: mosRows.length ? mosRows.reduce((sum, row) => sum + row.mos, 0) / mosRows.length : null, rows: rows.length, districts: new Set(rows.map((row) => row.reportingUnitKey)).size, provincialOffices: new Set(rows.filter((row) => row.reportingUnitType === "provincial_office").map((row) => row.reportingUnitKey)).size, stock: rows.reduce((sum, row) => sum + (Number(row.stock) || 0), 0), districtAmc: rows.filter((row) => row.reportingUnitType === "district").reduce((sum, row) => sum + (Number(row.amc) || 0), 0), provincialAmi: rows.filter((row) => row.reportingUnitType === "provincial_office").reduce((sum, row) => sum + (Number(row.amc) || 0), 0), stockouts: rows.filter((row) => row.stock === 0).length };
    });
    const vaccineProductRows = Object.values(vaccineRows.reduce((groups, row) => {
      const group = groups[row.vaccine] || { name: row.vaccine, rows: [], stock: 0, districtAmc: 0, provincialAmi: 0, stockouts: 0, expiring: 0, expired: 0 };
      group.rows.push(row); group.stock += row.stock || 0;
      if (row.reportingUnitType === "district") group.districtAmc += row.amc || 0;
      if (row.reportingUnitType === "provincial_office") group.provincialAmi += row.amc || 0;
      if (row.stock === 0) group.stockouts += 1;
      const expiry = vaccineExpiryDate(row.expiryDate);
      if (row.stock > 0 && expiry) {
        if (expiry < selectedPeriodEnd) group.expired += 1;
        else if (expiry <= expiryWindowEnd) group.expiring += 1;
      }
  ...
  return (
    <div className="control-tower-app">
      ...
      <aside className="dashboard-sidebar">
        ...
        <nav aria-label="Dashboard pages">
          {sidebarGroups.map((group) => {
            ...
            const hasActiveChild = group.programmeViews ? ["programmes", "vaccines"].includes(activePage)
            ...
            return <div className={`sidebar-nav-group ${isOpen ? "open" : ""}`} key={group.id}>
              ...
              {isOpen && <div className="sidebar-nav-children">
                ...
                {group.vaccineViews && <>
                  ...
                  {vaccineNavOpen && <div className="sidebar-vaccine-children">{[{ id: "overview", label: "Vaccine overview", icon: Syringe }, { id: "consumption", label: "District AMC trend", icon: ChartNoAxesCombined }, { id: "issues", label: "Provincial AMI trend", icon: Warehouse }, { id: "stock", label: "Stock on hand", icon: Boxes }, { id: "expiry", label: "Expiry monitoring", icon: CalendarDays }, { id: "reporting", label: "Reporting trend", icon: ClipboardCheck }, { id: "nonreporting", label: "Non-reporting", icon: FileX }, { id: "stockouts", label: "Stock-outs", icon: CircleX }].map((view) => {
      ...
      <main className={`app-shell dashboard-page page-${activePage} stock-${stockWorkspace} vaccine-${vaccineWorkspaceView}`}>
        ...
        <section className="table-panel">
          <div className="table-headline">
            <div>
              ...
              <h2 className="title-with-icon"><PackageSearch aria-hidden="true" />Facility-level commodity stock position</h2>
          ...
          {selectedCommodity && <>
            ...
            <div className="commodity-kpis">
              <button type="button" title="Facilities reporting this commodity in the selected scope. Click to see expected facilities that did not submit it." className={commodityStatusFilter === "all" ? "active" : ""} aria-pressed={commodityStatusFilter === "all"} onClick={() => { setCommodityStatusFilter("all"); setCommodityMissingOpen(true); }}><span><Users className="metric-icon" aria-hidden="true" />Facilities reporting <i className="kpi-help" aria-hidden="true">?</i></span><strong>{commodityKpiScopeRows.length}</strong><small>{commodityMissingFacilities.length} of {commodityExpectedFacilities.length} facilities did not report · click to review</small></button>
              <button type="button" title={`Availability is facilities with stock on hand above zero divided only by facilities that submitted this commodity. Reporting coverage is ${formatPercent(commodityReportingRate)}.`} className={commodityStatusFilter === "Available" ? "active" : ""} aria-pressed={commodityStatusFilter === "Available"} onClick={() => applyCommodityStatusFilter("Available")}><span><PackageSearch className="metric-icon" aria-hidden="true" />Commodity availability <i className="kpi-help" aria-hidden="true">?</i>{commodityReportingRate < 0.6 ? <i className="kpi-warning" title={`Caution: only ${formatPercent(commodityReportingRate)} of expected facilities submitted this commodity, so availability describes reporters rather than the full facility scope.`} aria-label="Low reporting coverage warning">!</i> : null}</span><strong>{formatPercent(commodityKpiScopeRows.length ? commodityKpiScopeRows.filter((row) => row.quantity > 0).length / commodityKpiScopeRows.length : 0)}</strong><small>{commodityKpiScopeRows.filter((row) => row.quantity > 0).length} available among {commodityKpiScopeRows.length} reporting facilities</small></button>
              <button type="button" title="Average months of stock across reporting facilities. MOS is stock on hand divided by average monthly consumption." className={commoditySort === "mos" && commodityStatusFilter === "all" ? "active" : ""} onClick={() => { setCommoditySort("mos"); applyCommodityStatusFilter("all"); }}><span><CalendarDays className="metric-icon" aria-hidden="true" />Average MOS <i className="kpi-help" aria-hidden="true">?</i></span><strong>{formatMos(commodityAverageMos)}</strong><small>Click to sort facilities by MOS</small></button>
              <button type="button" title="Stocked out: MOS is zero or below." className={`red ${commodityStatusFilter === "Stocked out" ? "active" : ""}`} aria-pressed={commodityStatusFilter === "Stocked out"} onClick={() => applyCommodityStatusFilter("Stocked out")}><span><CircleX className="metric-icon" aria-hidden="true" />Stocked out <i className="kpi-help" aria-hidden="true">?</i></span><strong>{commodityStatusCounts["Stocked out"] || 0}</strong><small>MOS = 0</small></button>
              <button type="button" title="Emergency stock: MOS is above zero and no more than 0.5 months." className={`amber ${commodityStatusFilter === "Emergency stock" ? "active" : ""}`} aria-pressed={commodityStatusFilter === "Emergency stock"} onClick={() => applyCommodityStatusFilter("Emergency stock")}><span><CircleAlert className="metric-icon" aria-hidden="true" />Emergency stock <i className="kpi-help" aria-hidden="true">?</i></span><strong>{commodityStatusCounts["Emergency stock"] || 0}</strong><small>Above 0 to 0.5 MOS</small></button>
              <button type="button" title="Understocked: MOS is above 0.5 and below 2 months." className={`amber ${commodityStatusFilter === "Understocked" ? "active" : ""}`} aria-pressed={commodityStatusFilter === "Understocked"} onClick={() => applyCommodityStatusFilter("Understocked")}><span><Gauge className="metric-icon" aria-hidden="true" />Understocked <i className="kpi-help" aria-hidden="true">?</i></span><strong>{commodityStatusCounts.Understocked || 0}</strong><small>Above 0.5 to below 2 MOS</small></button>
              <button type="button" title="According to plan: MOS is from 2 through 4 months." className={`green ${commodityStatusFilter === "According to plan" ? "active" : ""}`} aria-pressed={commodityStatusFilter === "According to plan"} onClick={() => applyCommodityStatusFilter("According to plan")}><span><CircleCheck className="metric-icon" aria-hidden="true" />According to plan <i className="kpi-help" aria-hidden="true">?</i></span><strong>{commodityStatusCounts["According to plan"] || 0}</strong><small>2 to 4 MOS</small></button>
              <button type="button" title="Overstocked: MOS is above 4 and below 12 months. Excess stock: MOS is 12 months or more." className={`blue ${commodityStatusFilter === "Overstocked / excess" ? "active" : ""}`} aria-pressed={commodityStatusFilter === "Overstocked / excess"} onClick={() => applyCommodityStatusFilter("Overstocked / excess")}><span><Boxes className="metric-icon" aria-hidden="true" />Overstocked / excess <i className="kpi-help" aria-hidden="true">?</i></span><strong>{(commodityStatusCounts.Overstocked || 0) + (commodityStatusCounts["Excess stock"] || 0)}</strong><small>Above 4 MOS</small></button>
            </div>
        ...
        <section className="vaccine-stock-section">
          <div className="weekly-head">
            <div>
              <p className="eyebrow dark">Vaccine Programme</p>
              <h2>Monthly vaccine stock counts</h2>
              <p>Provincial vaccine stock, average monthly issues (AMI), MOS, and stockout visibility from the programme officer's monthly workbook.</p>
            </div>
          </div>
          <div className="programme-source-strip vaccine-source-strip">
            <span><b>Source</b> {vaccineStockData.source}</span>
            <span><b>Coverage</b> {vaccineStockData.coverage}</span>
            <span className="programme-source-pending"><b>Scope</b> Separate programme stream; excluded from Tracer, Data Quality, and ZAMMSA totals</span>
          </div>
          ...
          <div className="vaccine-view vaccine-overview-view">
            <div className="stats-grid vaccine-kpis">
              ...
              <KpiCard icon={ChartNoAxesCombined} label="District AMC" value={Math.round(vaccineSummary.districtAmc).toLocaleString()} sub="Reported average monthly consumption" tone="amber" onClick={() => setVaccineWorkspaceView("consumption")} title="Open district AMC trend" />
              <KpiCard icon={Warehouse} label="Provincial AMI" value={Math.round(vaccineSummary.provincialAmi).toLocaleString()} sub="Reported average monthly issues" tone="blue" onClick={() => setVaccineWorkspaceView("issues")} title="Open provincial AMI trend" />
            ...
            <div className="table-panel vaccine-product-panel">
              <div className="table-headline"><div><h2>Antigens needing attention</h2><p>Lowest availability and MOS in {selectedVaccinePeriod?.label}.</p></div><span>{vaccineProductRows.length} vaccines</span></div>
              <div className="table-scroll"><table><thead><tr><th>Vaccine</th><th>Availability</th><th>Average MOS</th><th>Stock-outs</th><th>Expiry risk</th></tr></thead><tbody>{vaccineProductRows.slice(0, 10).map((row) => <tr key={row.name}><td><strong>{row.name}</strong></td><td>{formatPercent(row.availability)}</td><td>{formatMos(row.mos)}</td><td>{row.stockouts.toLocaleString()}</td><td>{row.expired ? `${row.expired} expired` : row.expiring ? `${row.expiring} expiring` : "No recorded risk"}</td></tr>)}</tbody></table></div>
            </div>
          </div>

          <div className="vaccine-view vaccine-consumption-view">
            <div className="stats-grid vaccine-kpis"><KpiCard icon={ChartNoAxesCombined} label="Current district AMC" value={Math.round(vaccineSummary.districtAmc).toLocaleString()} sub="District average monthly consumption" tone="amber" /><KpiCard icon={Gauge} label="Average MOS" value={formatMos(vaccineSummary.mos)} sub="Records with a submitted months-of-stock value" tone="amber" /><KpiCard icon={Boxes} label="Current stock" value={Math.round(vaccineSummary.stock).toLocaleString()} sub="Reported stock on hand" tone="green" /><KpiCard icon={MapPinned} label="Reporting districts" value={new Set(vaccineRows.filter((row) => row.reportingUnitType === "district").map((row) => row.reportingUnitKey)).size.toLocaleString()} sub="Districts with AMC records" /></div>
            <VaccineBarChart title="District AMC trend" subtitle="Reported district average monthly consumption across the selected scope." rows={vaccineTrendRows} value={(row) => row.districtAmc} label={(row) => row.label} onSelect={(row) => setVaccinePeriodId(row.id)} tone="amber" />
            <div className="table-panel"><div className="table-headline"><div><h2>District AMC trend</h2><p>Reported district average monthly consumption for the selected geography and antigen.</p></div></div><div className="table-scroll"><table><thead><tr><th>Month</th><th>District AMC</th><th>Stock on hand</th><th>Average MOS</th></tr></thead><tbody>{vaccineTrendRows.map((row) => <tr key={row.id} className={row.id === selectedVaccinePeriod?.id ? "selected-row" : ""}><td><button type="button" className="table-link-button" onClick={() => setVaccinePeriodId(row.id)}>{row.label}</button></td><td>{Math.round(row.districtAmc).toLocaleString()}</td><td>{Math.round(row.stock).toLocaleString()}</td><td>{formatMos(row.mos)}</td></tr>)}</tbody></table></div></div>
            <div className="table-panel"><div className="table-headline"><div><h2>District AMC by antigen</h2><p>Highest reported district average monthly consumption first.</p></div></div><div className="table-scroll"><table><thead><tr><th>Vaccine</th><th>District AMC</th><th>Stock on hand</th><th>MOS</th></tr></thead><tbody>{[...vaccineProductRows].sort((a, b) => b.districtAmc - a.districtAmc).map((row) => <tr key={row.name}><td><strong>{row.name}</strong></td><td>{Math.round(row.districtAmc).toLocaleString()}</td><td>{Math.round(row.stock).toLocaleString()}</td><td>{formatMos(row.mos)}</td></tr>)}</tbody></table></div></div>
          </div>

          <div className="vaccine-view vaccine-issues-view">
            <div className="stats-grid vaccine-kpis"><KpiCard icon={Warehouse} label="Current provincial AMI" value={Math.round(vaccineSummary.provincialAmi).toLocaleString()} sub="Provincial average monthly issues" tone="blue" /><KpiCard icon={MapPinned} label="Reporting PHOs" value={new Set(vaccineRows.filter((row) => row.reportingUnitType === "provincial_office").map((row) => row.reportingUnitKey)).size.toLocaleString()} sub="Provincial offices with AMI records" /><KpiCard icon={Boxes} label="Current stock" value={Math.round(vaccineSummary.stock).toLocaleString()} sub="Reported stock on hand" tone="green" /><KpiCard icon={Gauge} label="Average MOS" value={formatMos(vaccineSummary.mos)} sub="Submitted months of stock" tone="amber" /></div>
            <VaccineBarChart title="Provincial AMI trend" subtitle="Reported provincial average monthly issues across the selected scope." rows={vaccineTrendRows} value={(row) => row.provincialAmi} label={(row) => row.label} onSelect={(row) => setVaccinePeriodId(row.id)} tone="blue" />
            <div className="table-panel"><div className="table-headline"><div><h2>Provincial AMI trend</h2><p>Only PHO rows contribute to AMI. District AMC rows are excluded.</p></div></div><div className="table-scroll"><table><thead><tr><th>Month</th><th>Provincial AMI</th><th>Reporting PHOs</th></tr></thead><tbody>{vaccineTrendRows.map((row) => <tr key={row.id} className={row.id === selectedVaccinePeriod?.id ? "selected-row" : ""}><td><button type="button" className="table-link-button" onClick={() => setVaccinePeriodId(row.id)}>{row.label}</button></td><td>{Math.round(row.provincialAmi).toLocaleString()}</td><td>{row.provincialOffices.toLocaleString()}</td></tr>)}</tbody></table></div></div>
            <div className="table-panel"><div className="table-headline"><div><h2>Provincial AMI by antigen</h2><p>Only PHO rows contribute to these reported values.</p></div></div><div className="table-scroll"><table><thead><tr><th>Vaccine</th><th>Provincial AMI</th></tr></thead><tbody>{[...vaccineProductRows].sort((a, b) => b.provincialAmi - a.provincialAmi).map((row) => <tr key={row.name}><td><strong>{row.name}</strong></td><td>{Math.round(row.provincialAmi).toLocaleString()}</td></tr>)}</tbody></table></div></div>
          </div>

          <div className="vaccine-view vaccine-stock-view">
            <div className="stats-grid vaccine-kpis"><KpiCard icon={Boxes} label="Stock on hand" value={Math.round(vaccineSummary.stock).toLocaleString()} sub="Total reported doses" tone="green" /><KpiCard icon={Gauge} label="Average MOS" value={formatMos(vaccineSummary.mos)} sub="Submitted months of stock" tone="amber" /><KpiCard icon={CircleCheck} label="Availability" value={formatPercent(vaccineSummary.availability)} sub="Records with positive stock" tone="green" /><KpiCard icon={CircleX} label="Stock-outs" value={vaccineSummary.stockouts.toLocaleString()} sub="Zero stock records" tone={vaccineSummary.stockouts ? "red" : "green"} /></div>
            <VaccineBarChart title="Stock on hand by antigen" subtitle="Reported stock in the selected month, highest first." rows={[...vaccineProductRows].sort((a, b) => b.stock - a.stock)} value={(row) => row.stock} label={(row) => row.name} tone="green" onSelect={(row) => setVaccineName(row.name)} />
            <div className="table-panel"><div className="table-headline"><div><h2>Stock on hand by antigen</h2><p>District AMC and provincial AMI are kept separate.</p></div></div><div className="table-scroll"><table><thead><tr><th>Vaccine</th><th>Stock on hand</th><th>District AMC</th><th>Provincial AMI</th><th>Average MOS</th><th>Reporting records</th></tr></thead><tbody>{[...vaccineProductRows].sort((a, b) => b.stock - a.stock).map((row) => <tr key={row.name}><td><strong>{row.name}</strong></td><td>{Math.round(row.stock).toLocaleString()}</td><td>{Math.round(row.districtAmc).toLocaleString()}</td><td>{Math.round(row.provincialAmi).toLocaleString()}</td><td>{formatMos(row.mos)}</td><td>{row.rows.length.toLocaleString()}</td></tr>)}</tbody></table></div></div>
          </div>

          <div className="vaccine-view vaccine-expiry-view">
            <div className="stats-grid vaccine-kpis"><KpiCard icon={CalendarDays} label="Expired stock" value={vaccineSummary.expired.toLocaleString()} sub="Positive-stock records with a recorded expiry before the selected month" tone={vaccineSummary.expired ? "red" : "green"} /><KpiCard icon={CircleAlert} label="Expiring in 6 months" value={vaccineSummary.expiring.toLocaleString()} sub="Positive-stock records requiring FEFO review" tone={vaccineSummary.expiring ? "amber" : "green"} /><KpiCard icon={FileQuestion} label="No expiry captured" value={vaccineRows.filter((row) => row.stock > 0 && !vaccineExpiryDate(row.expiryDate)).length.toLocaleString()} sub="Positive-stock records with no usable expiry date" tone="neutral" /><KpiCard icon={Boxes} label="Records assessed" value={vaccineRows.filter((row) => row.stock > 0).length.toLocaleString()} sub="Positive-stock records in the selected filter" /></div>
            <VaccineBarChart title="Expiry risk summary" subtitle="Positive-stock records by expiry status in the selected month." rows={[{ id: "expired", name: "Expired", value: vaccineSummary.expired }, { id: "expiring", name: "Expiring in 6 months", value: vaccineSummary.expiring }, { id: "valid", name: "Later expiry", value: vaccineExpiryRows.filter((row) => row.expiryStatus === "Valid").length }]} value={(row) => row.value} label={(row) => row.name} tone="amber" />
            <div className="table-panel"><div className="table-headline"><div><h2>Expiry watchlist</h2><p>Earliest recorded expiry dates first. Expiry is assessed against the end of {selectedVaccinePeriod?.label}.</p></div><span>{vaccineExpiryRows.length} dated records</span></div><div className="table-scroll"><table><thead><tr><th>Status</th><th>Vaccine</th><th>Province</th><th>District</th><th>Stock on hand</th><th>Expiry date</th><th>VVM</th></tr></thead><tbody>{vaccineExpiryRows.slice(0, 120).map((row, index) => <tr key={`${row.vaccine}-${row.province}-${row.district}-${index}`}><td><span className={`priority-pill ${row.expiryStatus === "Expired" ? "critical" : row.expiryStatus.startsWith("Expiring") ? "warning" : "watch"}`}>{row.expiryStatus}</span></td><td><strong>{row.vaccine}</strong></td><td>{row.province}</td><td>{row.district}</td><td>{Math.round(row.stock).toLocaleString()}</td><td>{row.expiryDate}</td><td>{row.vvmStage ?? "-"}</td></tr>)}{!vaccineExpiryRows.length && <tr><td colSpan="7">No usable expiry dates are available in the selected vaccine records.</td></tr>}</tbody></table></div></div>
          </div>
          ...
          <div className="vaccine-view vaccine-stockouts-view">
            <div className="stats-grid vaccine-kpis"><KpiCard icon={CircleX} label="Current stock-outs" value={vaccineSummary.stockouts.toLocaleString()} sub="Zero stock records in the selected month" tone={vaccineSummary.stockouts ? "red" : "green"} /><KpiCard icon={Gauge} label="Average MOS" value={formatMos(vaccineSummary.mos)} sub="Use with stock-out count for prioritisation" tone="amber" /><KpiCard icon={MapPinned} label="Reporting districts" value={vaccineSummary.districts.toLocaleString()} sub="Scope represented in current records" /><KpiCard icon={Boxes} label="Stock on hand" value={Math.round(vaccineSummary.stock).toLocaleString()} sub="Total reported doses" tone="green" /></div>
            <VaccineBarChart title="Stock-out trend" subtitle="Reported zero-stock records in each monthly vaccine workbook." rows={vaccineTrendRows} value={(row) => row.stockouts} label={(row) => row.label} onSelect={(row) => setVaccinePeriodId(row.id)} tone="red" />
            <div className="table-panel"><div className="table-headline"><div><h2>Stock-out trend</h2><p>Monthly count of reported district-vaccine records with zero stock.</p></div></div><div className="table-scroll"><table><thead><tr><th>Month</th><th>Stock-outs</th><th>Availability</th><th>Records</th></tr></thead><tbody>{vaccineTrendRows.map((row) => <tr key={row.id} className={row.id === selectedVaccinePeriod?.id ? "selected-row" : ""}><td><button type="button" className="table-link-button" onClick={() => setVaccinePeriodId(row.id)}>{row.label}</button></td><td>{row.stockouts.toLocaleString()}</td><td>{formatPercent(row.availability)}</td><td>{row.rows.toLocaleString()}</td></tr>)}</tbody></table></div></div>
            <div className="table-panel"><div className="table-headline"><div><h2>Current stock-out register</h2><p>First 120 zero-stock records for focused follow-up.</p></div><span>{vaccineStockoutRows.length.toLocaleString()} current records</span></div><div className="table-scroll"><table><thead><tr><th>Vaccine</th><th>Province</th><th>Reporting unit</th><th>Metric</th><th>AMC / AMI</th><th>MOS</th><th>Expiry date</th></tr></thead><tbody>{vaccineStockoutRows.slice(0, 120).map((row, index) => <tr key={`${row.vaccine}-${row.province}-${row.district}-${index}`}><td><strong>{row.vaccine}</strong></td><td>{row.province}</td><td>{row.district}</td><td>{row.reportingUnitType === "provincial_office" ? "AMI" : "AMC"}</td><td>{Math.round(row.amc || 0).toLocaleString()}</td><td>{formatMos(row.mos)}</td><td>{row.expiryDate || "-"}</td></tr>)}{!vaccineStockoutRows.length && <tr><td colSpan="7">No stock-outs are recorded in the selected filter.</td></tr>}</tbody></table></div></div>
          </div>
        </section>
        ...
        <section className="reporting-rate-section">
          ...
          <div className="stats-grid reporting-kpi-grid">
            <KpiCard icon={MapPinned} label="Expected districts" value={reportingKpis.expected.toLocaleString()} sub="Click to show all expected districts" onClick={() => applyReportingKpiFilter("district-all", "all", "reporting-district-results")} active={reportKpiFilter === "district-all"} title="Show all expected districts" />
            <KpiCard icon={CircleCheck} label="DHO districts submitted" value={reportingKpis.reported.toLocaleString()} sub="Click to show districts with both Health Centre and Health Post received" onClick={() => applyReportingKpiFilter("district-reported", "Reported", "reporting-district-results")} active={reportKpiFilter === "district-reported"} title="Filter to submitted DHO districts" />
            <KpiCard icon={CircleAlert} label="Districts not complete" value={reportingKpis.notReported.toLocaleString()} sub={`${reportingKpis.partial} partial · ${reportingKpis.hospitalOnly} hospital-only · click to filter`} tone="red" onClick={() => applyReportingKpiFilter("district-missing", "Not Reported", "reporting-district-results")} active={reportKpiFilter === "district-missing"} title="Filter to incomplete or non-reporting districts" />
            <KpiCard icon={Activity} label="District reporting rate" value={formatPercent(reportingKpis.rate)} sub="Click to show all districts behind this rate" tone={reportingTone(reportingKpis.rate)} onClick={() => applyReportingKpiFilter("district-rate", "all", "reporting-district-results")} active={reportKpiFilter === "district-rate"} title="Show the districts behind the reporting rate" />
            <KpiCard icon={Users} label="Facilities expected" value={reportingFacilityKpis.expected.toLocaleString()} sub="Click to show all expected reporting units" onClick={() => applyReportingKpiFilter("facility-all", "all", "reporting-facility-results")} active={reportKpiFilter === "facility-all"} title="Show all expected facilities" />
            <KpiCard icon={FileCheck} label="Reports received" value={reportingFacilityKpis.received.toLocaleString()} sub="Click to show valid tracer submissions received" onClick={() => applyReportingKpiFilter("facility-reported", "Reported", "reporting-facility-results")} active={reportKpiFilter === "facility-reported"} title="Filter to facilities that reported" />
            <KpiCard icon={FileX} label="Reports missing" value={reportingFacilityKpis.missing.toLocaleString()} sub="Click to show expected units without a valid submission" tone={reportingFacilityKpis.missing ? "red" : undefined} onClick={() => applyReportingKpiFilter("facility-missing", "Not Reported", "reporting-facility-results")} active={reportKpiFilter === "facility-missing"} title="Filter to facilities that did not report" />
          </div>
        ...
        <section className="commodity-alert-centre">
          <div className="section-head">
            <div><p className="eyebrow dark">Commodity alert centre</p><h2 className="title-with-icon"><BellRing aria-hidden="true" />Warnings from current facility reports and forecast risk</h2><p>Critical alerts are current stockouts or a projected zero MOS. Warning and watch alerts identify commodities to validate before a stockout occurs.</p></div>
          ...
          <div className="stats-grid alert-kpis">
            <KpiCard icon={Siren} label="Critical" value={commodityAlertCounts.critical.toLocaleString()} sub="Immediate action: current stockout or projected zero MOS" tone={commodityAlertCounts.critical ? "red" : "green"} />
            <KpiCard icon={CircleAlert} label="Warning" value={commodityAlertCounts.warning.toLocaleString()} sub="Below 1 projected MOS or high forecast risk" tone={commodityAlertCounts.warning ? "amber" : "green"} />
            <KpiCard icon={BellRing} label="Watch" value={commodityAlertCounts.watch.toLocaleString()} sub="Early pressure needing monitoring" tone="blue" />
            <KpiCard icon={Users} label="Facilities affected" value={commodityAlertRows.reduce((total, row) => total + row.affectedFacilities, 0).toLocaleString()} sub="Risk rows connected to Facility Alerts" tone="neutral" />
          </div>
      ...
      {openCommodityFacility && <div className="commodity-detail-backdrop commodity-facility-drawer-backdrop" role="presentation" onMouseDown={() => setOpenCommodityFacility(null)}>
        <section className="commodity-detail-panel commodity-facility-drawer" role="dialog" aria-modal="true" aria-label="Facility commodity basket" onMouseDown={(event) => event.stopPropagation()}>
          ...
          <div className="commodity-detail-kpis">
            <div><span><Boxes className="metric-icon" aria-hidden="true" />Stock on hand</span><strong>{Math.round(openCommodityFacility.quantity).toLocaleString()}</strong></div>
            <div><span><Activity className="metric-icon" aria-hidden="true" />Average monthly consumption</span><strong>{Math.round(openCommodityFacility.amc).toLocaleString()}</strong></div>
            <div><span><CalendarDays className="metric-icon" aria-hidden="true" />Months of stock</span><strong>{formatMos(openCommodityFacility.mos)}</strong></div>
            <div className={commodityStatusTone(commodityStockStatus(openCommodityFacility.mos))}><span><ClipboardCheck className="metric-icon" aria-hidden="true" />Current classification</span><strong>{commodityStockStatus(openCommodityFacility.mos)}</strong></div>
          </div>
          <div className="commodity-history"><h3 className="title-with-icon"><Activity aria-hidden="true" />Submitted commodity trend</h3><p>Named facility records are shown first. Earlier combined primary-care records are labelled as district aggregates and are not interpreted as Health Post-only values.</p>{commodityFacilityHistory.length ? <div className="commodity-history-list">{commodityFacilityHistory.map((row) => <div key={row.reportDate}><span><CalendarDays className="history-icon" aria-hidden="true" />{row.label}</span><b>SOH {Math.round(row.quantity).toLocaleString()}</b><b>AMC {Math.round(row.amc).toLocaleString()}</b><b>{formatMos(row.mos)} MOS</b><em>{commodityStockStatus(row.mos)}</em></div>)}</div> : <div className="empty-state">No submitted historical record was found for this commodity and facility.</div>}</div>
