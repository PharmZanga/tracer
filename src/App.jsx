...
const moduleDescriptions = {
  ...
  vaccines: "Monthly vaccine stock, issues to districts, months of stock, and expiry monitoring.",
...
function App() {
  ...
  const vaccineWorkspace = useMemo(() => {
    ...
    const vaccineSummary = {
      ...
      consumption: vaccineRows.reduce((sum, row) => sum + (Number(row.amc) || 0), 0),
        ...
        <section className="vaccine-stock-section">
          <div className="weekly-head">
            <div>
              ...
              <p>District vaccine stock, provincial issues to districts, MOS, and stockout visibility from the programme officer's monthly workbook.</p>
            </div>
          </div>
          ...
          <div className="vaccine-view vaccine-overview-view">
            <div className="stats-grid vaccine-kpis">
              ...
              <KpiCard icon={ChartNoAxesCombined} label="Monthly issues" value={Math.round(vaccineSummary.consumption).toLocaleString()} sub="Reported issues to districts in the current filter" tone="amber" onClick={() => setVaccineWorkspaceView("consumption")} title="Open issues-to-districts trend" />
          ...
          <div className="vaccine-view vaccine-consumption-view">
            <div className="stats-grid vaccine-kpis"><KpiCard icon={ChartNoAxesCombined} label="Current issues" value={Math.round(vaccineSummary.consumption).toLocaleString()} sub="Reported provincial issues to districts this month" tone="amber" /><KpiCard icon={Gauge} label="Average MOS" value={formatMos(vaccineSummary.mos)} sub="Records with a submitted months-of-stock value" tone="amber" /><KpiCard icon={Boxes} label="Current stock" value={Math.round(vaccineSummary.stock).toLocaleString()} sub="Reported stock on hand" tone="green" /><KpiCard icon={MapPinned} label="Reporting units" value={vaccineSummary.districts.toLocaleString()} sub="Districts and PHOs with vaccine records" /></div>
            <VaccineBarChart title="Issues to districts trend" subtitle="Reported provincial issues to districts across the selected scope; this is not district consumption." rows={vaccineTrendRows} value={(row) => row.consumption} label={(row) => row.label} onSelect={(row) => setVaccinePeriodId(row.id)} tone="amber" />
            <div className="table-panel"><div className="table-headline"><div><h2>Issues to districts trend</h2><p>Monthly reported provincial issues to districts for the selected geography and antigen. These figures are not district consumption.</p></div></div><div className="table-scroll"><table><thead><tr><th>Month</th><th>Issues to districts</th><th>Stock on hand</th><th>Average MOS</th></tr></thead><tbody>{vaccineTrendRows.map((row) => <tr key={row.id} className={row.id === selectedVaccinePeriod?.id ? "selected-row" : ""}><td><button type="button" className="table-link-button" onClick={() => setVaccinePeriodId(row.id)}>{row.label}</button></td><td>{Math.round(row.consumption).toLocaleString()}</td><td>{Math.round(row.stock).toLocaleString()}</td><td>{formatMos(row.mos)}</td></tr>)}</tbody></table></div></div>
            <div className="table-panel"><div className="table-headline"><div><h2>Issues to districts by antigen</h2><p>Highest reported provincial issues first; this is not district consumption.</p></div></div><div className="table-scroll"><table><thead><tr><th>Vaccine</th><th>Issues to districts</th><th>Stock on hand</th><th>MOS</th></tr></thead><tbody>{[...vaccineProductRows].sort((a, b) => b.amc - a.amc).map((row) => <tr key={row.name}><td><strong>{row.name}</strong></td><td>{Math.round(row.amc).toLocaleString()}</td><td>{Math.round(row.stock).toLocaleString()}</td><td>{formatMos(row.mos)}</td></tr>)}</tbody></table></div></div>
          </div>
          ...
          <div className="vaccine-view vaccine-stock-view">
            ...
            <div className="table-panel"><div className="table-headline"><div><h2>Stock on hand by antigen</h2><p>Use this view to compare reported stock, provincial issues to districts, and months of stock.</p></div></div><div className="table-scroll"><table><thead><tr><th>Vaccine</th><th>Stock on hand</th><th>Issues to districts</th><th>Average MOS</th><th>District records</th></tr></thead><tbody>{[...vaccineProductRows].sort((a, b) => b.stock - a.stock).map((row) => <tr key={row.name}><td><strong>{row.name}</strong></td><td>{Math.round(row.stock).toLocaleString()}</td><td>{Math.round(row.amc).toLocaleString()}</td><td>{formatMos(row.mos)}</td><td>{row.rows.length.toLocaleString()}</td></tr>)}</tbody></table></div></div>
          </div>
          ...
          <div className="vaccine-view vaccine-stockouts-view">
            ...
            <div className="table-panel"><div className="table-headline"><div><h2>Current stock-out register</h2><p>First 120 zero-stock records for focused follow-up.</p></div><span>{vaccineStockoutRows.length.toLocaleString()} current records</span></div><div className="table-scroll"><table><thead><tr><th>Vaccine</th><th>Province</th><th>District</th><th>Issues to districts</th><th>MOS</th><th>Expiry date</th></tr></thead><tbody>{vaccineStockoutRows.slice(0, 120).map((row, index) => <tr key={`${row.vaccine}-${row.province}-${row.district}-${index}`}><td><strong>{row.vaccine}</strong></td><td>{row.province}</td><td>{row.district}</td><td>{Math.round(row.amc || 0).toLocaleString()}</td><td>{formatMos(row.mos)}</td><td>{row.expiryDate || "-"}</td></tr>)}{!vaccineStockoutRows.length && <tr><td colSpan="6">No stock-outs are recorded in the selected filter.</td></tr>}</tbody></table></div></div>
          </div>
        </section>
